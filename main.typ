#set page(paper: "a4", margin: 2.5cm)
#set text(font: ("New Computer Modern", "Noto Serif JP", "Yu Mincho", "BIZ UDPMincho", "MS Mincho"), size: 10.5pt)
#set par(justify: true, leading: 1.2em, first-line-indent: 1em)
#set heading(numbering: "1.")

#align(center)[
  #text(size: 16pt, weight: "bold")[空間操作が順序に依存しない場合の一般化]
  #v(1.5em)
]

= 一般化された定理

多次元格子空間上において、各格子点に値の未定義状態もしくは数値のいずれかの状態を取る格子が分布している状態を考える。この空間に対する操作が、各次元ごとの独立した変換の組み合わせとして表現できる場合、その次元ごとの操作を適用する順序は最終的な結果に影響を与えない。このような操作を「空間操作が順序に依存しない操作」と呼ぶ。

#v(1em)
#align(center)[
  #block(width: 100%, stroke: 1pt + black, inset: 1.5em, radius: 0.5em, align(left)[
    *定理（空間操作が順序に依存しない操作）* \
    空間の状態を $V$ とし、ある次元 $i$ に沿った操作を作用素 $D_i$ とする。以下の2条件を満たすとき、任意の2つの次元の操作（$D_i, D_j$）はどちらを先に行っても結果が等しい（$D_i compose D_j = D_j compose D_i$）。

    + *分離可能な評価関数* \
      ある起点 $p$ に値が存在する場合のみ、目的地のマス $x$ へ向かう変位ベクトル（正負の向きを持つ距離） $d = x - p$ および起点 $p$ に依存する関数 $f_i(d, p)$ を引き、「到達候補」となる値 $V(p) - f_i(d, p)$ を生成する。これは次元ごとに独立して計算できる。
    + *演算の可換・結合・分配法則* \
      複数の起点から同じマスに到達候補が集まった際、それらの集合を1つの値にまとめる集約演算 $plus.o$ について、計算の順番や組み合わせを変えても結果が変わらず（可換・結合的）、かつ「関数による変化量 $f_i(d, p)$ を引く」操作を後からまとめて行っても結果が等しい性質を持つ。
  ])
]
#v(1em)

= 未定義状態の扱いについて
例えば、集約演算が $max$ のときは空マスを $-oo$ とし、加算（$+$）のときは $0$ とみなす（すなわち演算の単位元をデフォルト値とする）方法は、適用する集約演算の種類によって空間の基本状態が依存してしまい、抽象化の観点からは最適とは言いがたい。

終了条件やデフォルト値の解釈に曖昧さが残るためである。これを解決するアプローチとして、空間を「各座標が単一の値を持つ関数」ではなく、「各座標が*値の集合*を持つ写像」として定義し直す手法が挙げられる。

- 値が存在しない状態は、特定のデフォルト値ではなく単なる「空集合 $emptyset$」として表す。
- 作用素 $D_i$ は、存在する値のみを移動・変換し、移動先の座標に「到達候補値の集合」を生成する。
- 集約演算 $plus.o$ は、要素が1つ以上ある集合に対してのみ適用され単一の値を返す。到着した集合が空（$emptyset$）の場合は、そのまま空集合となる。


= 定理の証明

分かりやすくするために、2次元空間を例に証明を行う。
X方向の操作を $D_X$、Y方向の操作を $D_Y$ とする。空間上の任意の起点を $(p, q)$、その初期状態の値を $V(p, q)$ とし、変位 $d$ および起点に依存する評価関数をそれぞれ $f_X, f_Y$ とする。

ある起点 $(p, q)$ の値が目的地 $(x, y)$ に到達するときの候補値は、$V(p,q) - f_X(x-p, p) - f_Y(y-q, q)$ のように計算される。

まず、$D_X$ を適用した後の状態 $V_X$ を考える。これは、同じ高さ（Y座標 $q$）に存在する全ての横方向（X座標 $p$）からの候補値の集合に対し、集約演算 $plus.o$ を適用したものである。
$V_X (x, q) = plus.o.big_p ( V(p, q) - f_X (x - p, p) )$

次に、その結果に対して $D_Y$ を適用する。今度は、同じ縦の列（X座標 $x$）に存在する全ての縦方向（Y座標 $q$）からの影響をまとめる。
$
  V_(X Y) (x, y) & = plus.o.big_q ( V_X (x, q) - f_Y (y - q, q) ) \
                 & = plus.o.big_q ( ( plus.o.big_p ( V(p, q) - f_X (x - p, p) ) ) - f_Y (y - q, q) )
$

ここで、定理の前提条件である「分配法則」を用いる。集約演算 $plus.o$ に対して「関数を引く」操作は分配できるため、外側にある $- f_Y(y-q, q)$ を内側のまとまりの中に入れることができる。
$V_(X Y) (x, y) = plus.o.big_q plus.o.big_p ( V(p, q) - f_X (x - p, p) - f_Y (y - q, q) )$

さらに、集約演算 $plus.o$ は順番を問わないため、縦と横のどちらから計算をまとめても、全座標 $(p, q)$ の集合から一斉に集約することと同じになる。また、関数の適用（$f_X$ と $f_Y$）も独立しているため順番を入れ替えられる。
$
  V_(X Y) (x, y) & = plus.o.big_(p, q) ( V(p, q) - f_X (x-p, p) - f_Y (y-q, q) ) \
                 & = plus.o.big_p plus.o.big_q ( ( V(p, q) - f_Y (y - q, q) ) - f_X (x - p, p) ) \
                 & = D_X [D_Y [V]] (x, y)
$

これにより、$D_X compose D_Y = D_Y compose D_X$ が示され、「XからY」の順でも「YからX」の順でも結果が一致することが証明された。
また、$f_X, f_Y$は任意の関数で成り立つ。

#pagebreak()

= 集約演算の分類

本体系において、ある格子座標に複数の起点からの到達候補値の集合が形成された場合、それらを1つの状態に決定するための「集約規則」が必要となる。ただし、操作の性質によってこの規則が不要な場合もある。

== 規則が不要な処理（単射な操作）
移動先が一意に決定され、複数の起点が同一の目的地にマッピングされない操作である。到達する候補値の集合の要素数が常に1以下になるため、集約演算が不要となる。例えば、図形全体の平行移動や縮小の操作など。

== 規則が必要な処理（単射ではない操作）
波紋のような値の伝播など、同じ格子に複数の値が到達し得る操作である。この場合、適用される演算子 $plus.o$ は可換法則および結合法則を満たす必要がある。例えば以下のようなものである。

- 最大値（$max$）や最小値（$min$）：評価関数が「減算」で定義される場合に分配法則を満たす。
- 加算（$+$）や乗算（$times$）：評価関数が「乗算」などで定義される場合に分配法則を満たす。
- 集合的演算：「平均値」や「最頻値」の算出。

#pagebreak()

= 具体例

本体系は幾何学的な操作から値の伝播までを包含できる。以下に $5 times 5$ の2次元空間における具体例を図示する。

#let cell-size = 2.5em
#let grid-table(..content) = table(
  columns: (cell-size, cell-size, cell-size, cell-size, cell-size),
  rows: (cell-size, cell-size, cell-size, cell-size, cell-size),
  align: center + horizon,
  ..content
)

== 平行移動
目標とする移動量を $k$ としたとき、関数 $f_i(d, p)$ を以下のような区分関数として定義する。
$
  f_i(d, p) = cases(
    0 & (d = k text(" のとき")),
    +oo & (d != k text(" のとき"))
  )
$
変位が $k$ 以外の場所では到着時の値が $V(p) - oo = -oo$ （未定義）となり、集合から取り除かれる。正負の向きを持つ $k$ を指定することで、任意の方向への平行移動が表現される。

#align(center)[
  #grid(
    columns: 3,
    gutter: 2em,
    [
      *初期状態* \


      #grid-table(
        [],
        [],
        [],
        [],
        [],
        [],
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *X方向へ $+1$ 移動* \


      #grid-table(
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *Y方向へ $-1$ 移動* \


      #grid-table(
        [],
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
  )
]

== 拡大
倍率を $s$ としたとき、起点のマスが $s$ 個のマスに複製されるよう、変位 $d$ と起点 $p$ を用いて関数を以下のように定義する。
$
  f_i(d, p) = cases(
    0 & (p(s-1) <= d < p(s-1) + s text(" のとき")),
    +oo & (text("それ以外"))
  )
$
この関数により、座標 $p$ にある値は $s p$ から $s p + s - 1$ までの範囲に複製され、値の入っているマスが増加する。

#align(center)[
  #grid(
    columns: 3,
    gutter: 2em,
    [
      *初期状態* \


      #grid-table(
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *X方向へ $2$ 倍拡大* \


      #grid-table(
        [1],
        [1],
        [2],
        [2],
        [],
        [3],
        [3],
        [4],
        [4],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *Y方向へ $2$ 倍拡大* \


      #grid-table(
        [1],
        [2],
        [],
        [],
        [],
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
  )
]

== 値の伝播
変位 $d$ の絶対値に比例して値が減少する評価関数を定義する。初期値を $10$ とし、X方向とY方向の評価関数をそれぞれ以下のように定義する。
$
  f_X(d, p) = 2|d|, quad f_Y(d, p) = 1|d|
$

#align(center)[
  #grid(
    columns: 3,
    gutter: 2em,
    [
      *初期状態* \


      #grid-table(
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [*10*],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *X方向へ伝播* \


      #grid-table(
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [6],
        [8],
        [*10*],
        [8],
        [6],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *Y方向へ伝播* \


      #grid-table(
        [4],
        [6],
        [8],
        [6],
        [4],
        [5],
        [7],
        [9],
        [7],
        [5],
        [6],
        [8],
        [*10*],
        [8],
        [6],
        [5],
        [7],
        [9],
        [7],
        [5],
        [4],
        [6],
        [8],
        [6],
        [4],
      )
    ],
  )
]

#pagebreak()

== 値伝播と移動の複合操作
起点が複数存在し、値が衝突して $max$ 演算で集約される伝播操作と、図形の平行移動を組み合わせたよりケース。初期値を $6$ とし、以下の3つの操作を定義する。いずれも到達候補値が $0$ 以下の場合は空集合として扱う。

$
  & "X方向伝播 " (D_X): quad f_X(d, p) = 2|d| \
  & "Y方向伝播 " (D_Y): quad f_Y(d, p) = 2|d| \
  & "Y方向移動 " (M): quad f_M(d, p) = cases(0 & (d = +1 text(" のとき")), +oo & (text("それ以外")))
$

これら3つの異なる演算を適用する順番を変えて、「$D_X -> M -> D_Y$」「$M -> D_Y -> D_X$」「$D_Y -> D_X -> M$」の3通りのルートで実行した結果を示す。中間の状態はそれぞれ異なるが、最終結果は完全に一致する。

#let cell-sm = 1.45em
#let grid-sm(..content) = table(
  columns: (cell-sm, cell-sm, cell-sm, cell-sm, cell-sm),
  rows: (cell-sm, cell-sm, cell-sm, cell-sm, cell-sm),
  align: center + horizon,
  ..content
)

#v(1em)
#align(center)[
  #grid(
    columns: (auto, auto, auto, auto, auto, auto, auto),
    align: center + horizon,
    gutter: 0.8em,

    // ルート1
    [*初期状態* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [],
        [*6*],
        [],
        [*6*],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(D_X)$ \ X伝播],
    [*状態1* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [4],
        [6],
        [4],
        [6],
        [4],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(M)$ \ Y移動],
    [*状態2* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [4],
        [6],
        [4],
        [6],
        [4],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(D_Y)$ \ Y伝播],
    [*最終結果* \


      #grid-sm(
        [],
        [2],
        [],
        [2],
        [],
        [2],
        [4],
        [2],
        [4],
        [2],
        [4],
        [6],
        [4],
        [6],
        [4],
        [2],
        [4],
        [2],
        [4],
        [2],
        [],
        [2],
        [],
        [2],
        [],
      )],

    grid.cell(colspan: 7)[#v(1em)],
    // Spacer

    // ルート2
    [*初期状態* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [],
        [*6*],
        [],
        [*6*],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(M)$ \ Y移動],
    [*状態1* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [*6*],
        [],
        [*6*],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(D_Y)$ \ Y伝播],
    [*状態2* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [],
        [4],
        [],
        [4],
        [],
        [],
        [6],
        [],
        [6],
        [],
        [],
        [4],
        [],
        [4],
        [],
        [],
        [2],
        [],
        [2],
        [],
      )],
    [$arrow(D_X)$ \ X伝播],
    [*最終結果* \


      #grid-sm(
        [],
        [2],
        [],
        [2],
        [],
        [2],
        [4],
        [2],
        [4],
        [2],
        [4],
        [6],
        [4],
        [6],
        [4],
        [2],
        [4],
        [2],
        [4],
        [2],
        [],
        [2],
        [],
        [2],
        [],
      )],

    grid.cell(colspan: 7)[#v(1em)],
    // Spacer

    // ルート3
    [*初期状態* \


      #grid-sm(
        [],
        [],
        [],
        [],
        [],
        [],
        [*6*],
        [],
        [*6*],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(D_Y)$ \ Y伝播],
    [*状態1* \


      #grid-sm(
        [],
        [4],
        [],
        [4],
        [],
        [],
        [6],
        [],
        [6],
        [],
        [],
        [4],
        [],
        [4],
        [],
        [],
        [2],
        [],
        [2],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(D_X)$ \ X伝播],
    [*状態2* \


      #grid-sm(
        [2],
        [4],
        [2],
        [4],
        [2],
        [4],
        [6],
        [4],
        [6],
        [4],
        [2],
        [4],
        [2],
        [4],
        [2],
        [],
        [2],
        [],
        [2],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(M)$ \ Y移動],
    [*最終結果* \


      #grid-sm(
        [],
        [2],
        [],
        [2],
        [],
        [2],
        [4],
        [2],
        [4],
        [2],
        [4],
        [6],
        [4],
        [6],
        [4],
        [2],
        [4],
        [2],
        [4],
        [2],
        [],
        [2],
        [],
        [2],
        [],
      )],
  )
]

#pagebreak()

= 座標依存の評価関数への拡張と経路独立性

これまでは、評価関数 $f_i$ が「変位 $d$」と「起点 $p$」のみに依存する均質な空間を前提としていた。しかし、地形の高低差や摩擦係数のように、場所によって移動コストが変化する非均質（Heterogeneous）な空間を表現するためには、評価関数が「移動先の座標」も参照できるように拡張する必要がある。

2次元空間において、X方向への評価関数を $f_X(d_X, x, y)$、Y方向への評価関数を $f_Y(d_Y, x, y)$ と再定義する。起点 $(p, q)$ から目的地 $(x, y)$ へ移動する際の2つのルートにおける減算量（コスト）を計算する。

*ルート1：X方向へ移動後、Y方向へ移動（$D_X -> D_Y$）*
+ 中間点 $(x, q)$ へのX方向のコスト：$f_X(x-p, x, q)$
+ 目的地 $(x, y)$ へのY方向のコスト：$f_Y(y-q, x, y)$
+ 合計コスト：$f_X(x-p, x, q) + f_Y(y-q, x, y)$

*ルート2：Y方向へ移動後、X方向へ移動（$D_Y -> D_X$）*
+ 中間点 $(p, y)$ へのY方向のコスト：$f_Y(y-q, p, y)$
+ 目的地 $(x, y)$ へのX方向のコスト：$f_X(x-p, x, y)$
+ 合計コスト：$f_Y(y-q, p, y) + f_X(x-p, x, y)$

空間操作が順序に依存しない（可換である）ためには、どの経路を辿っても到達候補値が一致しなければならない。すなわち、任意の $(p, q, x, y)$ において以下の等式が成り立つ必要がある。
$
  f_X(x-p, x, q) + f_Y(y-q, x, y) = f_Y(y-q, p, y) + f_X(x-p, x, y)
$
項を整理すると、以下の「可換性のための条件式」が得られる。
$
  f_X(x-p, x, y) - f_X(x-p, x, q) = f_Y(y-q, x, y) - f_Y(y-q, p, y)
$

== 直交分離による独立の証明

上記の条件式を満たす自明なケースとして、各方向の評価関数が互いの直交する軸の座標に依存しない場合が挙げられる。

X方向の評価関数 $f_X$ がY座標に依存せず、Y方向の評価関数 $f_Y$ がX座標に依存しないと仮定する。このとき、それぞれより低次元の関数 $g_X, g_Y$ を用いて以下のように表すことができる。
$
  f_X(d_X, x, y) & = g_X(d_X, x) \
  f_Y(d_Y, x, y) & = g_Y(d_Y, y)
$

この仮定を先ほどの可換性のための条件式に代入する。

左辺について、Y座標への依存がなくなるため以下のようになる。
$
  "左辺" & = g_X(x-p, x) - g_X(x-p, x) \
         & = 0
$

同様に右辺について、X座標への依存がなくなるため以下のようになる。
$
  "右辺" & = g_Y(y-q, y) - g_Y(y-q, y) \
         & = 0
$

したがって、任意の座標において $0 = 0$ となり、条件式が恒等的に成立することが示された。

*結論:* \
各次元の評価関数が、その移動方向と同じ軸の座標のみに依存し、他の直交する軸の座標に依存しない（直交分離可能である）場合、空間全体が均質でなくとも空間操作は順序に依存しない。物理的には「X軸方向への移動ルールは現在のX座標のみで決まり、Y方向の座標には影響されない」という独立した環境に相当する。

== 直交分離による独立の具体例

ここでは、評価関数が絶対座標に依存する（空間が非均質である）ものの、直交分離可能であるため操作順序に依存しない代表的な例を挙げる。これらの操作は、自身の移動方向の座標（$x$ または $y$）には依存するが、直交する他方の座標には依存しないため、前節の条件を満たす。

=== 1. ZoomOut関数（解像度維持のブロック集約）
格子空間において、空間サイズを維持したまま局所的なブロック内の値をすべて集約（ブロードキャスト）する操作を考える。ここでは $2 times 2$ のブロック内の値をすべて加算し、ブロック内の全マスに同じ合計値を格納する。

目的地の座標 $x$ と起点の座標 $p$（変位 $d_X = x - p$）が同じブロック $floor(x/2)$ に属している場合のみ値を取得するよう、評価関数を定義する。
$
  f_X(d_X, x, y) = cases(
    0 & (floor(x / 2) = floor((x - d_X) / 2) text(" のとき")),
    +oo & (text("それ以外"))
  )
$
この関数には $y$ が登場しない（Y座標によらずX方向の集約ルールは一定である）。そのため、各軸ごとに操作を分離可能であり、順序に依存しない。

=== 2. Extrude関数（空間の引き延ばし・押しつぶし）
絶対座標系に基づいて、空間の特定範囲を押しつぶしたり、引き延ばしたりする変形操作である。

例えば、X軸上の特定の範囲（$X_"start": 10, X_"end": 15$）を2倍に引き延ばし、それ以降のマスを右へシフトさせる操作を考える。目的地 $x$ に値が到達するための条件（変位 $d_X$）は、目的地 $x$ が「引き延ばし範囲より前か」「範囲内か」「範囲より後か」によって動的に変化する。
$
  f_X(d_X, x, y) = cases(
    0 & (x <= 10 text(" かつ ") d_X = 0),
    0 & (10 < x <= 20 text(" かつ 起点 ") p = 10 + floor((x - 10) / 2) text(" に対し ") d_X = x - p),
    0 & (x > 20 text(" かつ ") d_X = 5),
    +oo & (text("それ以外"))
  )
$
このように操作の振る舞いが目的地 $x$ の絶対座標によって切り替わる（空間が局所的に歪む）非均質な変換であっても、$f_X$ が $y$ に依存しないという性質は保たれている。したがって、X軸方向のExtrude操作と、Y軸方向に対する任意の操作は順序を入れ替えても最終的な結果に影響を与えない。

#pagebreak()

=== 直交分離操作の単体動作例
複合操作の前提として、新たに定義した「ZoomOut関数（ブロック集約）」および「Extrude関数（引き延ばし）」の各方向への単体での動作を図示する。

#let cell-size = 2.5em
#let grid-table(..content) = table(
  columns: (cell-size, cell-size, cell-size, cell-size, cell-size),
  rows: (cell-size, cell-size, cell-size, cell-size, cell-size),
  align: center + horizon,
  ..content
)

*1. ZoomOut関数（ブロック集約）の単体動作* \
空間サイズを維持したまま、$floor(x/2)$ または $floor(y/2)$ のブロックごとに値を加算集約（ブロードキャスト）する。

#align(center)[
  #grid(
    columns: 3,
    gutter: 2em,
    [
      *初期状態* \


      #grid-table(
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [],
        [5],
        [6],
        [],
        [],
        [],
        [7],
        [8],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *X方向の集約* ($Z_X$) \


      #grid-table(
        [3],
        [3],
        [],
        [],
        [],
        [7],
        [7],
        [],
        [],
        [],
        [],
        [],
        [11],
        [11],
        [],
        [],
        [],
        [15],
        [15],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *Y方向の集約* ($Z_Y$) \


      #grid-table(
        [4],
        [6],
        [],
        [],
        [],
        [4],
        [6],
        [],
        [],
        [],
        [],
        [],
        [12],
        [14],
        [],
        [],
        [],
        [12],
        [14],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
  )
]

#v(1.5em)

*2. Extrude関数（引き延ばし）の単体動作* \
絶対座標系において、インデックス `1`（左から2番目 / 上から2番目）の行・列を複製して2マスに引き延ばし、それ以降をシフトさせる。

#align(center)[
  #grid(
    columns: 3,
    gutter: 2em,
    [
      *初期状態* \


      #grid-table(
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *X方向の引き延ばし* ($E_X$) \


      #grid-table(
        [1],
        [2],
        [2],
        [3],
        [],
        [4],
        [5],
        [5],
        [6],
        [],
        [7],
        [8],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
    [
      *Y方向の引き延ばし* ($E_Y$) \


      #grid-table(
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )
    ],
  )
]

#pagebreak()

=== 図解：ZoomOut関数の可換性
ここでは $5 times 5$ の空間を用い、X方向・Y方向それぞれにおいて同じ $floor(x/2)$、$floor(y/2)$ のブロック内の値を「加算（$+$）」で集約する。
「X方向縮小（$Z_X$）」と「Y方向縮小（$Z_Y$）」の適用順序を入れ替えても、最終的な値の分布は完全に一致し、各ブロック内の全マスに同じ合計値が格納される。

#let cell-sm = 1.45em
#let grid-sm(..content) = table(
  columns: (cell-sm, cell-sm, cell-sm, cell-sm, cell-sm),
  rows: (cell-sm, cell-sm, cell-sm, cell-sm, cell-sm),
  align: center + horizon,
  ..content
)

#v(1em)
#align(center)[
  #grid(
    columns: (auto, auto, auto, auto, auto),
    align: center + horizon,
    gutter: 0.8em,

    // ルート1
    [*初期状態* \


      #grid-sm(
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [],
        [5],
        [6],
        [],
        [],
        [],
        [7],
        [8],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(Z_X)$ \ X縮小],
    [*状態1* \


      #grid-sm(
        [3],
        [3],
        [],
        [],
        [],
        [7],
        [7],
        [],
        [],
        [],
        [],
        [],
        [11],
        [11],
        [],
        [],
        [],
        [15],
        [15],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(Z_Y)$ \ Y縮小],
    [*最終結果* \


      #grid-sm(
        [10],
        [10],
        [],
        [],
        [],
        [10],
        [10],
        [],
        [],
        [],
        [],
        [],
        [26],
        [26],
        [],
        [],
        [],
        [26],
        [26],
        [],
        [],
        [],
        [],
        [],
        [],
      )],

    grid.cell(colspan: 5)[#v(1em)],

    // ルート2
    [*初期状態* \


      #grid-sm(
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [],
        [5],
        [6],
        [],
        [],
        [],
        [7],
        [8],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(Z_Y)$ \ Y縮小],
    [*状態1* \


      #grid-sm(
        [4],
        [6],
        [],
        [],
        [],
        [4],
        [6],
        [],
        [],
        [],
        [],
        [],
        [12],
        [14],
        [],
        [],
        [],
        [12],
        [14],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(Z_X)$ \ X縮小],
    [*最終結果* \


      #grid-sm(
        [10],
        [10],
        [],
        [],
        [],
        [10],
        [10],
        [],
        [],
        [],
        [],
        [],
        [26],
        [26],
        [],
        [],
        [],
        [26],
        [26],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
  )
]

#pagebreak()

=== 図解：Extrude関数の可換性
インデックス `1` の行および列を複製して2マスに引き延ばし、それ以降をシフトさせる操作（押し出されたマスは消滅する）を考える。

「X方向の引き延ばし（$E_X$）」と「Y方向の引き延ばし（$E_Y$）」のどちらを先に行っても、空間の歪み方および値の分布は最終的に一致する。

#v(1em)
#align(center)[
  #grid(
    columns: (auto, auto, auto, auto, auto),
    align: center + horizon,
    gutter: 0.8em,

    // ルート1
    [*初期状態* \


      #grid-sm(
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(E_X)$ \ X引延し],
    [*状態1* \


      #grid-sm(
        [1],
        [2],
        [2],
        [3],
        [],
        [4],
        [5],
        [5],
        [6],
        [],
        [7],
        [8],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(E_Y)$ \ Y引延し],
    [*最終結果* \


      #grid-sm(
        [1],
        [2],
        [2],
        [3],
        [],
        [4],
        [5],
        [5],
        [6],
        [],
        [4],
        [5],
        [5],
        [6],
        [],
        [7],
        [8],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
      )],

    grid.cell(colspan: 5)[#v(1em)],

    // ルート2
    [*初期状態* \


      #grid-sm(
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(E_Y)$ \ Y引延し],
    [*状態1* \


      #grid-sm(
        [1],
        [2],
        [3],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [4],
        [5],
        [6],
        [],
        [],
        [7],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(E_X)$ \ X引延し],
    [*最終結果* \


      #grid-sm(
        [1],
        [2],
        [2],
        [3],
        [],
        [4],
        [5],
        [5],
        [6],
        [],
        [4],
        [5],
        [5],
        [6],
        [],
        [7],
        [8],
        [8],
        [9],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
  )
]

#pagebreak()

=== 図解：異なる操作の複合（Extrude と ZoomOut の可換性）
直交する次元に対して異なる非均質な操作を適用するケースを検証する。
X方向には特定の列（インデックス `1`）を複製して引き延ばす「Extrude（$E_X$）」を適用し、Y方向には空間サイズを保ったまま $1 times 2$ の縦ブロックで値を集約する「ZoomOut（$Z_Y$）」を適用する。

操作の性質が全く異なっていても、互いの評価関数が直交する軸に依存しないため、適用順序を入れ替えても最終的な結果は完全に一致する。

#v(1em)
#align(center)[
  #grid(
    columns: (auto, auto, auto, auto, auto),
    align: center + horizon,
    gutter: 0.8em,

    // ルート1
    [*初期状態* \


      #grid-sm(
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [5],
        [],
        [],
        [],
        [],
        [6],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(E_X)$ \ X引延し],
    [*状態1* \


      #grid-sm(
        [1],
        [2],
        [2],
        [],
        [],
        [3],
        [4],
        [4],
        [],
        [],
        [],
        [5],
        [5],
        [],
        [],
        [],
        [6],
        [6],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(Z_Y)$ \ Y縮小],
    [*最終結果* \


      #grid-sm(
        [4],
        [6],
        [6],
        [],
        [],
        [4],
        [6],
        [6],
        [],
        [],
        [],
        [11],
        [11],
        [],
        [],
        [],
        [11],
        [11],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],

    grid.cell(colspan: 5)[#v(1em)],

    // ルート2
    [*初期状態* \


      #grid-sm(
        [1],
        [2],
        [],
        [],
        [],
        [3],
        [4],
        [],
        [],
        [],
        [],
        [5],
        [],
        [],
        [],
        [],
        [6],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(Z_Y)$ \ Y縮小],
    [*状態1* \


      #grid-sm(
        [4],
        [6],
        [],
        [],
        [],
        [4],
        [6],
        [],
        [],
        [],
        [],
        [11],
        [],
        [],
        [],
        [],
        [11],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
    [$arrow(E_X)$ \ X引延し],
    [*最終結果* \


      #grid-sm(
        [4],
        [6],
        [6],
        [],
        [],
        [4],
        [6],
        [6],
        [],
        [],
        [],
        [11],
        [11],
        [],
        [],
        [],
        [11],
        [11],
        [],
        [],
        [],
        [],
        [],
        [],
        [],
      )],
  )
]

